...
We want to make sure that we actually reject our null hypothesis in the case that it is false. That is, we would like to make sure that our test have high power: $P(\text{reject } H_0 | H_0 \text{ is false})$. The problem here is that $H_0$ can be wrong in many different way: it could be that the true $\mu$ is $2$, it could be $7$, it could be $4.26$. So to calculate power as it is actually formualted above is not really possible. However, we can calculate power for specific versions of "$H_0$ is false". 

Let's pretend that $H_0$ is false, and that the true value of $\mu$ is $\mu_A$. In this case, the power of our test is $P(\text{reject H_0} | \mu = \mu_A)$. As seen [above](#rejection-regions), $H_0$ is rejected when $\bar{x} > \mu_0 + z_{1 - \alpha / 2} \frac{\sigma}{\sqrt n}$ or $\bar{x} < \mu_0 + z_{\alpha / 2} \frac{\sigma}{\sqrt n}$, so the power of our test when $\mu = \mu_A$ is
...
Remember, $\bar{x} \sim \mathrm{Normal}\left(\mu, \frac{\sigma^2}{n}\right)$, so given that $\mu = \mu_A$, $\bar{x} \sim \mathrm{Normal}\left(\mu, \frac{\sigma^2}{n}\right)$, which let's us calculate the probabilities we need to find the power:
...
So, the power of our test, IF the true mean value is $\mu_A$, is 
...
Often times researchers like to go in the other direction: aim for a specific level of power for a specific difference between the null and the true parameter, and calculate how many observations are needed to reach that level. To achieve a power of $1 - \beta$ for a (two-sided) one sample Z-test, you need 
...
samples. If $n$ is not an integer, round up. Often, the denominator is thought of as the detectable difference. So, the question becomes how many samples are required to have sufficient power to detect a difference of size XX. 
...
Below is this same calculation done in `R`. Remember, $\beta = 1-\text{Power}$. Note the small descrepency. This is due to rounding error. 
