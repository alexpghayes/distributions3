...
with corresponding inference, predictions, residuals, etc. can be obtained in R
...
Both parameters have intuitive interpretations. First, the intercept
...
or equivalently using the `predict()` function:
...
associated expected probability distribution for zero to six goals:
...
in the tournament. We can easily set up the vector of all $128$ `Poisson()`
...
final that France won 4-2 against Croatia:
...
easily using the `outer()` function which by default performs a multiplication
of its arguments.
...
the model has a good marginal fit for this data. To bring out the deviations graphically
...
par(mar = c(4, 4, 1, 1))
bp <- barplot(sqrt(obsrvd), offset = sqrt(expctd) - sqrt(obsrvd),
  xlab = "Goals", ylab = "sqrt(Frequency)")
lines(bp, sqrt(expctd), type = "o", pch = 19, lwd = 2, col = 2)
abline(h = 0, lty = 2)
```
...
the derivative of the log-likelihood with respect to the paramter $\lambda$. The derivative
...
Unfortunately, when the paramter $\lambda$ is not constant but depends on a linear predictor
