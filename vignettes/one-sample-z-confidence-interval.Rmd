...
vignette: >
  %\VignetteIndexEntry{z-confidence-interval}
...
Again we will assume that $\sigma = 2$. Recall that a confidence interval for the mean based off the normal distribution is valid when:
...
If this is you, you have my condolences, and your instructor probably hates you. Ask that they use more standard notation.
...
lower_quantile_plot <- ggplot(data = NULL) +
  ...
  geom_text(
    aes(x = 3.2, y = 0.3, label = "z[0.95] == 1.96"),
    parse = TRUE,
    size = 4
  ) +
  labs(
...
upper_quantile_plot <- ggplot(data = NULL) +
  ...
  geom_vline(xintercept = 1.96, size = 1, color = "darkgrey") +
  geom_text(
    aes(x = 3.2, y = 0.3, label = "z[0.05] == 1.96"),
    parse = TRUE,
    size = 4
  ) +
  labs(
