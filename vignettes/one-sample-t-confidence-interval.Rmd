...
lower_quantile_plot <- ggplot(data = NULL) +
  ...
  geom_text(
    aes(x = 3.2, y = 0.3, label = "t[n-1, 0.95] == 1.83"),
    parse = TRUE,
    size = 4
  ) +
  labs(
    title = "Lower tail quantile of a T distribution (df = 9)",
...
upper_quantile_plot <- ggplot(data = NULL) +
  ...
  geom_vline(xintercept = 1.83, size = 1, color = "darkgrey") +
  geom_text(
    aes(x = 3.2, y = 0.3, label = "t[n-1, 0.05] == 1.83"),
    parse = TRUE,
    size = 4
  ) +
  labs(
    title = "Upper tail quantile of a T distribution (df = 9)",
